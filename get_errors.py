...
SCRIPT_NAME = 'CloudGenix: Example script: Checking Error Codes'
SCRIPT_VERSION = "v1"
...
def go():
    ...
    for sites in cgx.get.sites().cgx_content['items']:
        ...
        print("Checking site " + sites['name'])
        
        data = {"limit":{"count":100,"sort_on":"time","sort_order":"descending"},"view":{"summary":False},"end_time":"2022-03-16T12:41:37.354Z","start_time":"2021-09-16T12:41:37.354Z","severity":[],"priority":[],"query":{"site":[sites['id']],"category":[],"code":["DEVICESW_FPS_LIMIT_EXCEEDED","DEVICESW_CONCURRENT_FLOWLIMIT_EXCEEDED"],"correlation_id":[],"type":["alarm"]}}
        for event in cgx.post.events_query(data=data).cgx_content['items']:
            site_data = {}
            site_data['Site_Name'] = site_id2n[event['site_id']]
            site_data['Code'] = event['code']
            time = event['_created_on_utc'].split(".")
            site_data['Time'] = time[0]
            site_list.append(site_data)
    
    
    csv_columns = ['Site_Name','Code', 'Time']
    csv_file = "errors_results.csv"
    try:
